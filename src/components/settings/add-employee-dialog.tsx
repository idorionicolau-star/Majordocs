...
export function AddEmployeeDialog({ companyId, onEmployeeAdded, disabled }: AddEmployeeDialogProps) {
  ...
  const onSubmit = async (values: AddEmployeeFormValues) => {
    if (!companyId || !firestore) {
      toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível identificar a sua empresa. Tente novamente.' });
      return;
    }
    ...
    try {
      ...
      onEmployeeAdded(); // This will trigger a re-fetch in the parent
      form.reset();
      setOpen(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Crie as credenciais de acesso para um membro da sua equipa.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>

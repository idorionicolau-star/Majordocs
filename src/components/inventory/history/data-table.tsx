...
export function HistoryDataTable<TData extends StockMovement, TValue>({
  columns,
  data,
    ...
    <Card className="glass-card shadow-sm overflow-hidden">
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className="px-4 sm:px-8 py-4 text-xs">
      ...
      <CardFooter className="flex items-center justify-between space-x-2 p-4 border-t dark:border-slate-800/50">
        <div className="text-sm text-muted-foreground">
            {table.getFilteredRowModel().rows.length} de {data.length} registo(s).
        </div>

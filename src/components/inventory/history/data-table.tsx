...
interface DataTableProps<TData extends StockMovement, TValue> {
  columns: ColumnDef<TData, TValue>[]
  data: TData[]
}

export function MovementsDataTable<TData extends StockMovement, TValue>({
  columns,
  data,
    ...
    <Card className="glass-card shadow-sm overflow-hidden">
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow key={headerGroup.id} className="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
                  {headerGroup.headers.map((header) => (
                    <TableHead key={header.id} className="px-4 sm:px-6 py-4 text-[11px] font-[800] text-slate-400 uppercase tracking-[0.1em]">
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className="px-4 sm:px-6 py-3 text-sm">
      ...
      <CardFooter className="flex items-center justify-between space-x-2 p-4 border-t dark:border-slate-800/50">
        <div className="text-sm text-muted-foreground">
            {table.getRowModel().rows.length} de {data.length} registo(s).
        </div>
